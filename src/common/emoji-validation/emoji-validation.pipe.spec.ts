...
describe('EmojiValidationPipe', () => {
  ...
  it('should throw a BadRequest error if the value is less than zero', () => {
    const result = () => emojiPipe.transform(-1);
    expect(result).toThrow(BadRequestException);
  });
  it('should throw a BadRequest error if the value is greater than 43', () => {
    const result = () => emojiPipe.transform(50);
    expect(result).toThrow(BadRequestException);
  });
